...
module.exports = {
  ...
  login: function (req, res){
    passport.authenticate('local', function(err, user, info){
      if((err) || (!user)){
        return res.json([400],{ message: info.message,
        user
        });
      }
      req.login(user, function(err){
        ...
        return res.json([200],{ message: 'User Login Successful', user, token});
      })
    })(req, res);
  },
  ...
  register: function(req, res){
    ...
    User.create(data).fetch().exec(function(err, user){
      ...
      req.login(user, function(err){
        ...
        return res.json([200],{ message: 'New User Registration Successful'});
        // return res.redirect('/');
      })
    })
  }
